...
-- Forward reference to evalFunctionBody in Eval (breaks circular dependency).
-- Must be initialized by Eval.runModuleMain before any closures are applied.
evalFunctionBodyRef :: IORef (Env -> FunctionBody -> IO Value)
evalFunctionBodyRef = unsafePerformIO (newIORef (\_ _ -> error "Runtime: evalFunctionBodyRef not initialized"))
{-# NOINLINE evalFunctionBodyRef #-}
...
apply (VClosure cenv params body) args = do
  ...
  if length used < length params
    then do
      ...
      let env' = bindParams cenv (zip params used)
      evalFB <- readIORef evalFunctionBodyRef
      val <- evalFB env' body
