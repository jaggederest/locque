...
- CLI: `locque-interpreter run <file>` (type check + run .lq/.lqs), `typecheck <file>`, `emit-lqs <in.lq> <out.lqs>`, `emit-lq <in.lqs> <out.lq>`, `validate <file.lqs>`, `dump (core|normalized|elaborated|typed|typed-normalized|types|types-normalized) <file> [name]`. Smyth tool: `smyth run <file>` (type check + run), `smyth test` (all via test/main.lq), `smyth test <file>` (one), `smyth bench` (runs test/bench.lq), `smyth dump (core|normalized|elaborated|typed|typed-normalized|types|types-normalized) <file> [name]`. Installed at `~/.local/bin/smyth`.
